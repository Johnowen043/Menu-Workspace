...
FocusScope {
    ...
    function currentRow() {
        ...
        kicker.currentRow = Math.floor(currentIndex / Math.floor(width / itemGrid.cellWidth));
        return kicker.currentRow;
    }
    function currentCol() {
        if (currentIndex === -1) { return -1;}
        kicker.currentColumn=currentIndex - (currentRow() * Math.floor(width / itemGrid.cellWidth));
        return kicker.currentColumn;
    }
    function lastRow() {
    ...
    DropArea {
        ...
        onPositionChanged: event =>
        {
            ...
            if (item)
            {
                ...
                else if (kicker.dragSource.GridView.view.model.favoritesModel === itemGrid.model && !itemGrid.model.isFavorite(kicker.dragSource.favoriteId))
                {
            ...
            else if (kicker.dragSource.parent !== gridView.contentItem && kicker.dragSource.GridView.view.model.favoritesModel === itemGrid.model && !itemGrid.model.isFavorite(kicker.dragSource.favoriteId))
            {
                var hasPlaceholder = (itemGrid.model.dropPlaceholderIndex !== -1);
        ...
        PlasmaComponents.ScrollView {
            ...
            GridView {
                ...
                delegate: ItemGridDelegate { showLabel: itemGrid.showLabels }
                highlight: Item
                {
